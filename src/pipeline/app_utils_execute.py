...
def _temp_run(in_outs, generation, debug, result, return_output):
    try:
        if debug:
...
if __name__ == "__main__":
    ...
    # Test mbpp converted data
    data = json.load(open("../resources/data/code/mbpp-sanitized-256-var-replace-exec-app-style.json"))
    ...
    for item in tqdm(data):
        try:
            ...
            outputs = []
            for o in item["input_output"]["outputs"]:
                o = eval(o)
                if isinstance(o, tuple):
                    o = list(o)
                outputs.append(o)
            item["input_output"]["outputs"] = outputs
        ...
        full_res = check_correctness(item["input_output"], item["code"], timeout=2, debug=False)
        res = all(full_res)
        if res:
            cnt += 1
        else:
            print(full_res)
            print(item['code'])
            print(item["input_output"]["inputs"])
            print(item["input_output"]["outputs"])
            print("=====================================================================================")
